...
We will see a quick setup to configure the ``nextcloud`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``nextcloud`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``nextcloud`` formulas under ``/srv/salt/nextcloud/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/nextcloud/parameters/dns:domain/
    ln -s dns:domain /srv/salt/nextcloud/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/nextcloud/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/nextcloud/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``nextcloud`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``nextcloud`` formulas under ``/srv/salt/nextcloud/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/nextcloud/parameters/roles
...
We create a configuration for the role ``nextcloud/server`` in ``/srv/salt/nextcloud/parameters/roles/nextcloud/server.yaml``:
...
We create another configuration for the role ``nextcloud/client`` in ``/srv/salt/nextcloud/parameters/roles/nextcloud/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles nextcloud/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles nextcloud/client
...
.. code-block:: console
    ...
    mv /srv/salt/nextcloud/parameters/dns:domain/  '/srv/salt/nextcloud/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-nextcloud-as-json:
      test.nop:
        - name: {{ nextcloud | json }}
...
Here is an example based on `template-formula/nextcloud/config/file.sls`_:
...
This ``sls`` file expose a ``nextcloud`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ nextcloud['winner'] }}
...
.. _template-formula/nextcloud/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/nextcloud/config/file.sls
